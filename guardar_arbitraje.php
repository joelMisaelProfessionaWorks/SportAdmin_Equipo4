...
try {
    ...
    $concepto_pago = "Arbitraje (Jornada/Parcialidad $parcialidad)";

    if (empty($equipo_id) || empty($monto) || empty($parcialidad) || empty($fecha)) {
        throw new Exception("Faltan datos para registrar el pago de arbitraje.");
    }

    $id_usuario = 1; 

    // Insertamos en registro de abono como estaba antes, por compatibilidad
    ...
    echo json_encode([
        'success' => true,
        'mensaje' => 'Pago de arbitraje registrado correctamente.',
        'transaccion' => [
            'tipo_pago' => 'ARBITRAJE',
            'folio' => $folio_txn ?: 'ARB-' . time(),
